...
export default function LanguageSelector() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('sehatsaarthi_lang') || localStorage.getItem('aarogyanet_lang');
    if (saved && saved !== 'en') {
  ...
  const handleSelectLanguage = (code) => {
    ...
    localStorage.setItem('sehatsaarthi_lang', code);
  ...
  return (
    <div className="relative inline-block text-left z-50 notranslate" translate="no" ref={dropdownRef}>
      ...
      <button
        ...
        className="h-9 sm:h-10 px-2.5 sm:px-3.5 flex items-center gap-1.5 sm:gap-2 bg-white border-2 border-slate-200 hover:border-amber-400 rounded-xl text-slate-800 text-xs font-extrabold hover:bg-slate-50 transition-all shadow-xs"
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-[calc(100vw-28px)] max-w-[320px] sm:w-84 sm:max-w-none bg-white border-2 border-amber-400 rounded-2xl sm:rounded-3xl shadow-2xl z-[9999] p-3 animate-fadeIn flex flex-col max-h-[400px] sm:max-h-[460px] overflow-hidden drop-shadow-2xl">
          {/* Header */}
